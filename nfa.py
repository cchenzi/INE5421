class NFA:
    def __init__(self, states, alphabet, init_state, final_states, transitions, has_epsilon):
        ...
        if has_epsilon: self.epsilon_closure = dict.fromkeys(transitions)
    ...
    def compute_epsilon_closure(self):
        for k, tr in self.transitions.items():
            aux = []
            aux.append(k)
            if tr['&'] not in aux: aux.append(tr['&'])
            for y in aux:
    ...
    def determinize_epsilon(self):
        ...
        new_states = {}
        new_states[aux] = get_epsilon_transitions(aux)